...
const NavItem: React.FC<{ children: React.ReactNode, label: string, onClick?: () => void, active?: boolean, isProminent?: boolean }> = ({ children, label, onClick, active, isProminent }) => (
  <button
    onClick={onClick}
    className={`flex items-center w-full p-3 my-1 rounded-lg transition-all duration-200 group-hover:space-x-4
      ...
      <div className="mt-auto">
        {user ? (
          ...
          <NavItem label="Sign In" onClick={onLoginClick}>
            <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 16l-4-4m0 0l4-4m-4 4h14m-5 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h5a3 3 0 013 3v1" /></svg>
          </NavItem>
        )}
      </div>
    </aside>
