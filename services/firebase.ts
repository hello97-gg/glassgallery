...
export const toggleImageLike = async (image: ImageMeta, user: User) => {
    ...
    try {
        // Use a simpler approach: get the current state, compute new values, then update
        ...
        // Update only the like-related fields
        await imageRef.update({
            likedBy: newLikedBy,
            likeCount: newLikedBy.length,
        });

        // Phase 2: If this was a "like" action, create a notification
        if (wasLike && user.uid !== data.uploaderUid) {
            try {
